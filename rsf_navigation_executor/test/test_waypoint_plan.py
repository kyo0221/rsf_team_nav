...
def make_wp(checkpoint=False):
    return {'x': 0.0, 'y': 0.0, 'yaw': 0.0, 'checkpoint': checkpoint, 'speed_limit': 0.0}


def test_split_legs_no_checkpoint():
    waypoints = [make_wp(), make_wp(), make_wp()]
...
def test_split_legs_middle_checkpoint():
    a, b, c, d, e = make_wp(), make_wp(True), make_wp(), make_wp(True), make_wp()
...
def test_split_legs_leading_checkpoint():
    a, b, c = make_wp(True), make_wp(), make_wp()
...
def test_split_legs_trailing_checkpoint():
    a, b, c = make_wp(), make_wp(), make_wp(True)
    legs = split_legs([a, b, c])
    assert len(legs) == 1
    assert legs[0].start_index == 0
...
def test_split_legs_consecutive_checkpoints():
    a, b, c = make_wp(True), make_wp(True), make_wp()
    legs = split_legs([a, b, c])
    assert len(legs) == 3
    assert legs[0].start_index == 0
    assert legs[0].waypoints == [a]
